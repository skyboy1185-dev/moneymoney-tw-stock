...
const INDUSTRIES: Record<string, string> = { "24":"半導體", "25":"電腦及週邊設備", "26":"光電", "27":"通信網路", "28":"電子零組件", "29":"電子通路", "30":"資訊服務", "31":"其他電子" };
const cache: { value: unknown; expiresAt: number } = { value: null, expiresAt: 0 };
...
async function universe() {
  ...
  const companies: Company[] = [
    ...listedCompanies.map((row)=>{const symbol=text(row,["公司代號"]),code=text(row,["產業別"]),isElectronic=Boolean(INDUSTRIES[code]);return {symbol,name:text(row,["公司簡稱"]),market:"上市" as const,code,industry:INDUSTRIES[code]??subIndustry(symbol,"跨產業題材"),listingDate:rocDate(text(row,["上市日期"])),isElectronic};}),
    ...otcCompanies.map((row)=>{const symbol=text(row,["SecuritiesCompanyCode"]),code=text(row,["SecuritiesIndustryCode"]),isElectronic=Boolean(INDUSTRIES[code]);return {symbol,name:text(row,["CompanyAbbreviation"]),market:"上櫃" as const,code,industry:INDUSTRIES[code]??subIndustry(symbol,"跨產業題材"),listingDate:rocDate(text(row,["DateOfListing"])),isElectronic};}),
  ].filter((row)=>(row.isElectronic||isExpandedThemeSymbol(row.symbol)) && /^\d{4,6}$/.test(row.symbol));
  ...
  return companies.map((company)=>({company,quote:quotes.get(company.symbol)}))
...
function features(meta:StockMeta, prices:DailyPrice[], quote: QuoteRow, market20:number, electronic20:number) {
  ...
  return { meta, prices, stock:{
    stock_code:meta.symbol,stock_name:meta.name,market_type:meta.market,industry_code:(meta as StockMeta & {industryCode?:string}).industryCode!,main_industry:meta.industry,sub_industry:subIndustry(meta.symbol,meta.industry),listing_date:(meta as StockMeta & {listingDate?:string}).listingDate,
    ...
    return_1d:pct(closes,1),return_5d:pct(closes,5),return_20d:ret20,market_return_20d:market20,electronic_return_20d:electronic20,relative_strength_market:ret20-market20,relative_strength_electronic:ret20-electronic20,
...
async function buildAdaptiveElectronicScanUncached() {
  ...
  cache.value=payload;cache.expiresAt=Date.now()+60_000;return payload;
}

export async function buildAdaptiveElectronicScan() {
  if(cache.value && cache.expiresAt>Date.now())return cache.value;
  if(!scanInFlight){
    scanInFlight=buildAdaptiveElectronicScanUncached().finally(()=>{scanInFlight=null;});
  }
  return scanInFlight;
}
