...
const enrichmentCache = new Map<string, { expiresAt: number; values: Record<string, number | null> }>();
...
function features(meta:StockMeta, prices:DailyPrice[], quote: QuoteRow, market20:number, electronic20:number) {
  ...
  return { meta, prices, stock:{
    ...
    quote_source:quote.source,quote_timestamp:`${quote.date}T${quote.time}+08:00`,quote_realtime:Boolean(quote.realtime),price:latest.close,open:latest.open,high:latest.high,low:latest.low,volume_shares:latest.volume,average_volume_20d_shares:avgVol,average_turnover_20d:avgTurn,illiquid_days_5d:prices.slice(-5).filter((p)=>p.volume<1000).length,
...
async function buildAdaptiveElectronicScanUncached(scope: ScanScope) {
  ...
  const enrichment = new Map<string, Record<string, number | null>>();
  const enrich=valid.slice().sort((a,b)=>avg(b.prices.slice(-20).map((p)=>p.close*p.volume))-avg(a.prices.slice(-20).map((p)=>p.close*p.volume))).slice(0,25).map((item)=>({meta:item.meta,stock:{price:item.quote.price} as Record<string, number | null>}));
  await Promise.all(enrich.map(async(item)=>{const cached=enrichmentCache.get(item.meta.symbol);if(cached && cached.expiresAt>Date.now()){Object.assign(item.stock,cached.values);return;}try{const flow=await getOfficialStockInstitutionalFlow(item.meta);const last=flow.items.slice(-5);item.stock.foreign_net_5d=last.reduce((s,x)=>s+x.foreign,0);item.stock.trust_net_5d=last.reduce((s,x)=>s+x.trust,0);}catch{} try{const f=await enrichOfficialStockMeta(item.meta,item.stock.price ?? undefined);item.stock.trailing_eps=f.eps;}catch{} try{const h=await backendJson<LargeHolderHistoryResponse>(`/large-holders/stocks/${item.meta.symbol}/history?weeks=2`);if(h.dataMode==="official_tdcc"&&h.items.length>=2){item.stock.holder_400_change=h.items.at(-1)!.ratioOver400-h.items.at(-2)!.ratioOver400;item.stock.holder_1000_change=h.items.at(-1)!.ratioOver1000-h.items.at(-2)!.ratioOver1000;}}catch{}}));
  for (const item of enrich) { const extra = { ...item.stock }; delete extra.price; enrichment.set(item.meta.symbol, extra); if (!enrichmentCache.has(item.meta.symbol) || enrichmentCache.get(item.meta.symbol)!.expiresAt <= Date.now()) enrichmentCache.set(item.meta.symbol, { values: extra, expiresAt: Date.now()+300_000 }); }
  ...
  for (const item of base) Object.assign(item.stock, enrichment.get(item.meta.symbol));
  ...
  scanCaches[scope].value=payload;scanCaches[scope].expiresAt=Date.now()+(scope === "adaptive" ? 2_000 : 60_000);return payload;
}
...
export async function buildAdaptiveElectronicScan() {
  ...
  if(!scanInFlight.adaptive){
    scanInFlight.adaptive=withScanLoadLock(() => buildAdaptiveElectronicScanUncached("adaptive"))
      .catch((error)=>{
        // A failed refresh must not serve an expired trading snapshot.
        throw error;
      })
