...
- 個股分析、AI 選股、超強AI當沖系統與獨立 `/day-trading-bot` 當沖機器人
...
- AI選股與 AI當沖只掃描 AI 供應鏈／低軌衛星主題聯集。AI 供應鏈包含 PCB、ABF 載板、被動元件、記憶體、玻纖布、IC 設計，股票池與子題材標籤統一由 `services/theme-stock-universe.ts` 管理
- LINE Messaging API 群組通知管理卡，可查看遮罩群組 ID、推送狀態、測試與解除綁定

個股卡片會標示官方報價來源；盤中大小單使用後端 Fugle 成交明細推估，
並在畫面標示動態門檻、推估值與排除的集合競價／盤後成交。
