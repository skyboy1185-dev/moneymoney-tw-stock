...
export function DayTradingDashboard() {
  ...
  const quoteCoverageCount = regime.quoteCoverageCount ?? regime.supervisor?.quoteCoverageCount ?? 0;
  const candidateUniverseCount = regime.candidateUniverseCount ?? regime.supervisor?.candidateUniverseCount ?? 0;
  const indexDelayWarning = regime.dataQualityMode === "index_delay"
    ? regime.dataQualityWarning
      ?? `加權指數延遲 ${regime.dataDelaySeconds} 秒；個股即時報價覆蓋 ${quoteCoverageCount}/${candidateUniverseCount}，正式訊號仍以個股逐檔風控。`
    : null;
  const formalBlockReason = regime.formalBlockReason
    ?? (!regime.automation.formalSignalsAllowed && regime.marketOpen ? regime.automation.statusMessage : null);
  ...
  return <div className="adaptive-page day-trading-page day-trading-adaptive">
    ...
    {regime.degraded && regime.dataQualityMode !== "index_delay" && <div className="data-anomaly-banner"><ShieldAlert /><div><strong>當沖核心資料降級，正式進場已暫停</strong><span>{regime.fallbackReason ? `錯誤摘要：${regime.fallbackReason}` : "系統保留頁面與既有監控狀態，等待後端核心資料恢復。"}</span></div></div>}
    <p className="pattern-risk-notice">
      <ShieldAlert />策略隔離：本頁是「當沖機器人」，使用原版盤中訊號、VWAP、5 分 K、大單與即時持倉監控及 day_trading_* 績效；可共用行情資料，但不沿用「超強AI當沖系統」的 SUPER_AI_DAYTRADE AI 評分、空方候選池或獨立績效。
    </p>
    ...
    <section className="adaptive-heading dt-hero">
      <div><span className="eyebrow"><Radio size={13} /> {regime.mode === "official" ? "MARKET DATA · SIGNALS ONLY" : regime.mode === "warming_up" ? "MARKET DATA WARMING UP" : "MOCK STREAMING · SIGNALS ONLY"}</span><h1>AI 當沖多空機器人</h1><p>共用大單動能雷達 {regime.supervisor?.candidateUniverseCount ?? 300} 檔・行情覆蓋 {regime.supervisor?.quoteCoverageCount ?? 0}/{regime.supervisor?.candidateUniverseCount ?? 300}・5 分 K 完成 {regime.supervisor?.warmedSymbolCount ?? 0}・高頻追蹤 {regime.supervisor?.highFrequencyTrackingCount ?? 0}</p></div>
    ...
    {regime.automation.phase === "long_only" && <div className="automation-banner phase-long-only"><Activity /><div><strong>{regime.automation.schedule.latestEntryTime} 後停止新進場</strong><span>多空都不再新增部位；既有持倉仍依 VWAP、5 分 K、量能、大單與停損停利持續管理。</span></div></div>}
    ...
    <section className="dt-direction-policy">
      <article className="long"><div><span>多方進場時段</span><strong>{regime.automation.schedule.signalStartTime}～{regime.automation.schedule.longEntryCutoffTime}</strong></div><b>{regime.automation.schedule.longEntryCutoffTime} 後不新增</b><p>{regime.automation.schedule.longEntryCutoffTime} 前接受符合風控或平衡試單條件的做多訊號。既有多單仍可即時停損、停利、減碼；符合隔夜條件時依原規則管理。</p></article>
      <article className="short"><div><span>空方進場時段</span><strong>{regime.automation.schedule.signalStartTime}～{regime.automation.schedule.shortEntryCutoffTime}</strong></div><b>{regime.automation.schedule.shortEntryCutoffTime} 後不新增</b><p>{regime.automation.schedule.shortEntryCutoffTime} 後禁止新增空單；既有空單仍持續停損與停利監控，並於收盤前完成回補。</p></article>
    ...
    <section className="adaptive-table-card live-signal-section">
      <div className="table-title"><div><h2>本小時 AI 當沖精選</h2><p>多空新進場皆只到 {regime.automation.schedule.latestEntryTime}。嚴格正式單為 0 時，最多開 2 檔平衡試單。</p></div><span>{signals.length}／{regime.maximumRecommendations} 檔</span></div>
      {signals.length ? <div className="adaptive-table-wrap"><table><thead><tr><th>排名</th><th>股票</th><th>方向／確認模式</th><th>評分</th><th>目前價</th><th>進場區間</th><th>停損／目標</th><th>風報比／大單</th><th>狀態</th><th>操作</th></tr></thead><tbody>{signals.map((signal) => <tr key={signal.id}><td>#{signal.rank}</td><td><b>{signal.symbol}</b><span>{signal.stockName} · {signal.market}</span></td><td><b className={signal.direction === "long" ? "text-up" : "text-down"}>{signal.direction === "long" ? "做多" : "放空"}</b><span>{signal.entryConfirmationModeLabel ?? signal.threeGateEntryStatus ?? signal.threeGateStatus ?? signal.action}</span></td><td><b>{signal.confidenceScore}</b><span>健康 {signal.healthScore}</span></td><td>{displayNumber(signal.price)}<span>{signal.changePercent >= 0 ? "+" : ""}{displayNumber(signal.changePercent)}%</span></td><td>{displayNumber(signal.entryMin)}～{displayNumber(signal.entryMax)}<span>{signal.vwapStatus}</span></td><td>{displayNumber(signal.stopLoss)}<span>{displayNumber(signal.target1)}／{displayNumber(signal.target2)}</span></td><td>{displayNumber(signal.riskRewardRatio)}<span>{signal.largeOrderStatus ?? `${displayNumber(signal.largeOrderForce, 0)} 分`}</span></td><td><span className={`candidate-status ${signal.status}`}>{signal.recommendationLabel || signal.status}</span><small>{signal.starterReason ?? signal.qualificationFailures?.[0]}</small></td><td><div className="row-actions"><button onClick={() => setSelectedSignalId(signal.id)}><Eye size={14} />詳情</button><button onClick={() => monitor(signal)}>監控</button><button onClick={() => void simulate(signal)}>模擬</button></div></td></tr>)}</tbody></table></div> : <div className="adaptive-empty">{regime.automation.phase === "scanning" ? "目前沒有符合風控標準的交易機會，建議觀望。" : regime.automation.statusMessage}</div>}
    </section>
    ...
    <section className="adaptive-table-card live-signal-section">
      ...
      {candidates.length ? <div className="adaptive-table-wrap"><table><thead><tr><th>排名</th><th>股票</th><th>方向</th><th>信心／確認</th><th>健康／動能</th><th>大單</th><th>RR／流動性</th><th>狀態與刷掉原因</th></tr></thead><tbody>{candidates.slice(0, 20).map((candidate) => {
        const blocker = candidate.starterReason ?? candidate.qualificationFailures?.[0] ?? candidate.starterQualificationFailures?.[0] ?? "符合正式推薦條件";
        return <tr key={candidate.id}><td>#{candidate.rank}</td><td><b>{candidate.symbol}</b><span>{candidate.stockName} · {candidate.market}</span></td><td><b className={candidate.direction === "long" ? "text-up" : "text-down"}>{candidate.direction === "long" ? "做多" : "放空"}</b><span>{candidate.entryConfirmationModeLabel ?? candidate.action}</span></td><td><b>{displayNumber(candidate.confidenceScore, 0)} / {displayNumber(candidate.confirmationScore, 0)}</b><span>正式門檻 70 / 35</span></td><td><b>{displayNumber(candidate.healthScore, 0)}</b><span>{candidate.vwapStatus}</span></td><td><b>{displayNumber(candidate.largeOrderForce, 0)}</b><span>{candidate.largeOrderStatus ?? (candidate.largeOrderContinuousBuy || candidate.largeOrderContinuousSell ? "連續性通過" : "連續性不足")}</span></td><td><b>{displayNumber(candidate.riskRewardRatio)}</b><span>量 {displayNumber(candidate.volume, 0)}</span></td><td><span className={`candidate-status ${candidate.isOfficialRecommendation ? "confirmed" : "blocked"}`}>{candidate.isOfficialRecommendation ? "正式可進場" : "候選未通過"}</span><small>{blocker}</small></td></tr>;
...
function CandidateReplaySection({
  ...
  return <section className="adaptive-table-card live-signal-section">
    ...
    {items.length ? <div className="adaptive-table-wrap"><table><thead><tr><th>時間 / 當時排名</th><th>股票</th><th>方向</th><th>新版判斷</th><th>信心 / 確認</th><th>大單動能</th><th>健康 / RR</th><th>刷掉原因</th></tr></thead><tbody>{items.slice(0, 80).map((item) => {
      ...
      return <tr key={`${item.snapshotAt}-${item.id}`}><td><b>{displayTime(item.snapshotAt)}</b><span>#{item.rank}</span></td><td><b>{item.symbol}</b><span>{item.stockName} · {item.market}</span></td><td><b className={item.direction === "long" ? "text-up" : "text-down"}>{item.direction === "long" ? "多" : "空"}</b><span>{item.entryConfirmationModeLabel ?? item.action}</span></td><td><span className={`candidate-status ${item.wouldBeOfficialRecommendation ? "confirmed" : "blocked"}`}>{item.wouldBeOfficialRecommendation ? "會列正式" : "不列正式"}</span><small>原本：{item.originalOfficialRecommendation ? "正式" : "候選"}</small></td><td><b>{displayNumber(item.confidenceScore, 0)} / {displayNumber(item.confirmationScore, 0)}</b><span>正式門檻 70 / 35</span></td><td><b>{displayNumber(item.largeOrderForce, 0)}</b><span>{item.largeOrderStatus ?? (item.largeOrderContinuousBuy || item.largeOrderContinuousSell ? "連續大單成立" : "連續大單不足")}</span></td><td><b>{displayNumber(item.healthScore, 0)} / {displayNumber(item.riskRewardRatio)}</b><span>量 {displayNumber(item.volume, 0)}</span></td><td><small>{failure}</small></td></tr>;
