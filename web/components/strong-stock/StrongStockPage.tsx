...
export function StrongStockPage({ userId, onSelectStock }: { userId: string; onSelectStock: (symbol: string) => void }) {
  ...
  return <section className="strong-page">
    ...
    {section === "overview" && <>
      ...
      {!!data.pendingOrders.length && <article className="strong-panel"><header><CandlestickChart size={17} /><div><strong>隔日模擬委託</strong><small>需有新鮮五檔賣一與足夠掛單量才會模擬成交</small></div></header><div className="strong-table"><table><thead><tr><th>股票</th><th>型態</th><th>限價</th><th>委託／已成交／剩餘</th><th>狀態</th><th>有效日</th><th>原因</th></tr></thead><tbody>{data.pendingOrders.map((row) => <tr key={row.id}><td><b>{row.symbol}</b><small>{row.name}</small></td><td>{statusLabel(row.entryType)}</td><td>{n(row.limitPrice, 2)}</td><td>{n(row.quantity)}／{n(row.filledQuantity)}／{n(row.remainingQuantity)}</td><td>{row.status === "PARTIALLY_FILLED" ? "部分成交" : "等待成交"}</td><td>{row.validDate}</td><td>{row.reason || "—"}</td></tr>)}</tbody></table></div></article>}
    </>}
    ...
    {section === "positions" && <article className="strong-panel"><header><WalletCards size={17} /><div><strong>獨立模擬持倉</strong><small>新版成本依新鮮賣一、可見量及滑價撮合；舊資料會明確標示</small></div></header><div className="strong-table"><table><thead><tr><th>股票</th><th>模擬成交時間／依據</th><th>成本／現價</th><th>股數／資金</th><th>未實現損益</th><th>停損／移動停利</th><th>下次加碼</th><th>評分</th><th>理由／警告</th><th>操作</th></tr></thead><tbody>{data.positions.map((row) => <tr key={row.id}><td><b>{row.symbol}</b><small>{row.name}｜{row.industry}</small></td><td>{dt(row.entryAt)}<small>{row.executionModel === "LEGACY_LAST_TRADE" ? "舊版簡化撮合" : `${row.execution.quoteSource ?? "五檔"}｜賣一 ${row.execution.bestAsk ?? "—"}｜${row.execution.fillStatus === "PARTIAL" ? "部分成交" : "全數成交"}`}</small></td><td>{n(row.averageCost, 2)}<small>{n(row.currentPrice, 2)}｜v{row.strategyVersion}</small></td><td>{n(row.quantity)}股<small>{n(row.investedCapital)}元</small></td><td className={tone(row.unrealizedPnl)}>{signed(row.unrealizedPnl)}元<small>{signed(row.returnPct)}%</small></td><td>{n(row.initialStop, 2)}<small>{n(row.trailingStop, 2)}</small></td><td>{row.nextAddPrice ? n(row.nextAddPrice, 2) : "—"}</td><td>{n(row.currentScore, 1)}</td><td>{row.reasons.join("、") || "—"}<small className="warn">{row.warnings.join("、")}</small></td><td><button className="danger-soft" disabled={busy} onClick={() => { if (window.confirm(`確認以目前顯示價格模擬賣出 ${row.symbol}？`)) void run(() => strongStockClient.close(userId, row.id, row.currentPrice, "使用者手動模擬賣出"), `${row.symbol} 已完成模擬賣出`); }}>模擬賣出</button></td></tr>)}</tbody></table></div>{!data.positions.length && <p className="strong-empty">目前沒有強勢股模擬持倉</p>}</article>}
