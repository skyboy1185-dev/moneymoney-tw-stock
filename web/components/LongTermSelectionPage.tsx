      ...
      <section className="long-term-positions">
        ...
        {!data.items.length ? <div className="long-term-waiting">
          ...
          <tbody>{data.items.map((item, index) => <tr key={item.id}>
            ...
            <td><strong>{price(item.currentPrice)}</strong><small>{({ live: "最新成交價（定時更新）", delayed: "延遲／備援報價", close: "日收盤價", stored: "留存估值", unknown: "報價時間不明" } as Record<string, string>)[item.quoteStatus ?? "unknown"]}</small><small>{item.quoteSource ?? "來源待確認"}</small><small>{item.quoteTimestamp ? dateTime(item.quoteTimestamp) : "無可驗證的報價時間"}</small></td>
