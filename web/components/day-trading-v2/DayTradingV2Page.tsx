...
function BacktestPerformance({ job, robotById }: { job: Record<string, unknown>; robotById: Map<string, string> }) {
  ...
  const research = record(result.regimeResearch);
  if (research) return <RegimeResearchResult data={research} robotById={robotById} />;
...
type OptimizationDetail = { kind: "HEALTH" | "JOB" | "CHALLENGER"; title: string; data: Record<string, unknown> };

function RegimeResearchResult({ data, robotById }: { data: Record<string, unknown>; robotById: Map<string, string> }) {
  const names: Record<string, string> = { A_STRONG_TREND: "強勢趨勢", B_MILD_BULL: "溫和多頭", C_RANGE: "區間震盪", D_WEAK: "弱勢", E_CRASH: "急跌／異常" };
  const labels: Record<string, string> = { BASELINE: "原策略", NET_RR_1: "扣成本後賺賠比 ≥ 1", SELECTIVE: "較嚴格量能與停利" };
  const candidateName = (value: unknown) => {
    const row = record(value);
    return row ? `${robotById.get(String(row.strategyId)) ?? row.strategyId} · ${labels[String(row.id).split(":")[1]] ?? "候選參數"}` : "空手（未通過門檻）";
  };
  return <div className="dt2-backtest-performance">
    ...
    {list(data.folds).map((fold, index) => {
      ...
      const tested = record(fold.outOfSampleRegimes) ?? {};
      ...
      return <section key={index}>
        ...
        <div className="dt2-table"><table><thead><tr><th>盤勢</th><th>測試前凍結的選擇</th><th>驗證淨損益／筆數</th><th>測試淨損益／筆數</th></tr></thead><tbody>
          {Object.entries(names).map(([id, name]) => {
            ...
            const test = performanceFrom(tested[id]);
...
function list(value: unknown): Array<Record<string, unknown>> {
  return Array.isArray(value) ? value.flatMap((item) => { const parsed = record(item); return parsed ? [parsed] : []; }) : [];
}

function OptimizationDetailPanel({ detail, onClose }: { detail: OptimizationDetail; onClose: () => void }) {
  if (detail.kind === "HEALTH") {
    const items = list(detail.data.items);
    return <article className="dt2-panel">
      <header><History size={17} /><strong>{detail.title}</strong><button onClick={onClose}>關閉</button></header>
      <div className="dt2-table"><table><thead><tr><th>診斷日</th><th>版本</th><th>狀態</th><th>最近20筆</th><th>最近50筆</th><th>原因</th><th>措施</th><th>資金／風險</th></tr></thead><tbody>{items.map((item, index) => {
        const metrics = record(item.metrics) ?? {}; const last20 = record(metrics.last20) ?? {}; const last50 = record(metrics.last50) ?? {};
        return <tr key={String(item.id ?? index)}><td>{String(item.diagnosisDate ?? "—")}</td><td>v{String(item.strategyVersion ?? "—")}</td><td>{String(item.status ?? "—")}</td><td className={tone(String(last20.netPnl ?? 0))}>{signedMoney(String(last20.netPnl ?? 0))}<small>{String(last20.tradeCount ?? 0)}筆｜PF {String(last20.profitFactor ?? "—")}</small></td><td className={tone(String(last50.netPnl ?? 0))}>{signedMoney(String(last50.netPnl ?? 0))}<small>回撤 {String(last50.maxDrawdown ?? "—")}</small></td><td>{Array.isArray(item.reasons) ? item.reasons.join("、") : "—"}</td><td>{String(item.recommendedAction ?? "—")}</td><td>{String(item.capitalMultiplier ?? "—")}／{String(item.riskMultiplier ?? "—")}</td></tr>;
      })}</tbody></table></div>
      {!items.length && <p className="dt2-empty">尚無歷史診斷；執行「立即診斷」後會開始保留每日紀錄。</p>}
    </article>;
  }

  if (detail.kind === "JOB") {
    const trials = list(detail.data.trials);
    return <article className="dt2-panel">
      <header><SlidersHorizontal size={17} /><strong>{detail.title}</strong><button onClick={onClose}>關閉</button></header>
      <p className="dt2-data-warning">每組候選參數的驗證與樣本外績效均會永久保存。只有通過全部門檻的候選版能進入 Challenger 模擬。</p>
      <div className="dt2-table"><table><thead><tr><th>候選</th><th>狀態</th><th>參數</th><th>驗證淨損益／PF</th><th>樣本外淨損益／PF</th><th>未通過原因</th></tr></thead><tbody>{trials.map((trial, index) => {
        const validation = record(trial.validationMetrics) ?? {}; const oos = record(trial.oosMetrics) ?? {};
        return <tr key={String(trial.id ?? index)}><td>#{String(trial.candidateIndex ?? index + 1)}{Boolean(trial.selected) && <small>已選中</small>}</td><td className={Boolean(trial.passed) ? "gain" : "warn"}>{String(trial.status ?? "—")}</td><td><details><summary>查看參數</summary><pre className="dt2-result">{JSON.stringify(trial.parameters ?? {}, null, 2)}</pre></details></td><td className={tone(String(validation.netPnl ?? 0))}>{signedMoney(String(validation.netPnl ?? 0))}<small>PF {String(validation.profitFactor ?? "—")}｜{String(validation.tradeCount ?? 0)}筆</small></td><td className={tone(String(oos.netPnl ?? 0))}>{signedMoney(String(oos.netPnl ?? 0))}<small>PF {String(oos.profitFactor ?? "—")}｜{String(oos.tradeCount ?? 0)}筆</small></td><td>{Array.isArray(trial.failures) ? trial.failures.join("、") || "—" : "—"}</td></tr>;
      })}</tbody></table></div>
      {!trials.length && <p className="dt2-empty">任務尚未產生候選試驗，或此為功能更新前建立的任務。</p>}
    </article>;
  }

  const trades = list(detail.data.trades); const events = list(detail.data.events);
  const champion = record(detail.data.championMetrics) ?? {}; const challenger = record(detail.data.challengerMetrics) ?? {};
  return <article className="dt2-panel">
    <header><Bot size={17} /><strong>{detail.title}</strong><button onClick={onClose}>關閉</button></header>
    <div className="dt2-metric-grid compact"><article><span>Champion 淨損益</span><strong className={tone(String(champion.netPnl ?? 0))}>{signedMoney(String(champion.netPnl ?? 0))}</strong><small>{String(champion.tradeCount ?? 0)}筆｜勝率 {String(champion.winRate ?? "—")}%</small></article><article><span>Challenger 淨損益</span><strong className={tone(String(challenger.netPnl ?? 0))}>{signedMoney(String(challenger.netPnl ?? 0))}</strong><small>{String(challenger.tradeCount ?? 0)}筆｜勝率 {String(challenger.winRate ?? "—")}%</small></article><article><span>完整交易日</span><strong>{String(detail.data.fullTradingDays ?? 0)}日</strong></article><article><span>錯誤</span><strong>{String(detail.data.errorCount ?? 0)}次</strong></article></div>
    <h3>模擬完成交易</h3><div className="dt2-table"><table><thead><tr><th>角色</th><th>股票</th><th>進場／出場</th><th>價格／股數</th><th>成本</th><th>淨損益</th></tr></thead><tbody>{trades.map((trade, index) => <tr key={String(trade.id ?? index)}><td>{String(trade.role ?? "—")}</td><td>{String(trade.symbol ?? "—")}</td><td>{dateTime(String(trade.entryTime ?? ""))}<small>{dateTime(String(trade.exitTime ?? ""))}</small></td><td>{number(String(trade.entryPrice ?? 0), 2)} → {number(String(trade.exitPrice ?? 0), 2)}<small>{number(String(trade.quantity ?? 0))}股</small></td><td>{number(String(trade.cost ?? 0))}元</td><td className={tone(String(trade.netPnl ?? 0))}>{signedMoney(String(trade.netPnl ?? 0))}</td></tr>)}</tbody></table></div>
    {!trades.length && <p className="dt2-empty">尚無已完成的模擬交易。</p>}
    <h3>訊號、淘汰與錯誤紀錄</h3><div className="dt2-table"><table><thead><tr><th>時間</th><th>角色</th><th>事件</th><th>股票</th><th>明細</th></tr></thead><tbody>{events.map((event, index) => <tr key={String(event.id ?? index)}><td>{dateTime(String(event.occurredAt ?? ""))}</td><td>{String(event.role ?? "—")}</td><td>{String(event.eventType ?? "—")}</td><td>{String(event.symbol ?? "—")}</td><td><details><summary>查看</summary><pre className="dt2-result">{JSON.stringify(event.payload ?? {}, null, 2)}</pre></details></td></tr>)}</tbody></table></div>
    {!events.length && <p className="dt2-empty">尚無事件，或此為功能更新前建立的模擬。</p>}
  </article>;
}
...
export function DayTradingV2Page({ userId }: { userId: string }) {
  ...
  const researchJob = backtestHistory.find((job) => Boolean(record(record(job.result)?.regimeResearch)));
  ...
  async function showOptimizationDetail(kind: OptimizationDetail["kind"], title: string, action: () => Promise<Record<string, unknown>>) {
    setBusy(true); setError("");
    try { setOptimizationDetail({ kind, title, data: await action() }); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "讀取優化紀錄失敗"); }
    finally { setBusy(false); }
  }
  ...
  return <section className="dt2-page">
    ...
    {section === "backtest" && <div className="dt2-grid two">
      {researchJob && <article className="dt2-panel" style={{ gridColumn: "1 / -1" }}><BacktestPerformance job={researchJob} robotById={robotById} /></article>}
