...
const dateTime = (value: string) => new Date(value).toLocaleString("zh-TW", { hour12: false });
...
export function AIStockWorkflow({
  ...
  const confirmEntry = async (item: AIStockMonitor) => {
    ...
    await perform(`/ai-stock-monitor/${item.id}/confirm-entry`, {
      ...
      custom_stop_loss: item.stopLoss, line_exit_notifications: true, add_on_enabled: true,
    });
  };
  ...
  const confirmAddOn = async (item: AIStockPosition) => {
    ...
    await perform(`/ai-stock-positions/${item.id}/confirm-add-on`, {
      ...
      add_on_time: new Date().toISOString(), fee: 0, accept_new_stop_loss: true,
    });
  };
  ...
  const partialExit = async (item: AIStockPosition) => {
    ...
    await perform(`/ai-stock-positions/${item.id}/partial-exit`, {
      quantity, exit_price: exitPrice, exit_time: new Date().toISOString(), fee: 0, tax: 0,
    });
  };
  ...
  const closePosition = async (item: AIStockPosition) => {
    ...
    await perform(`/ai-stock-positions/${item.id}/close`, {
      ...
      exit_time: new Date().toISOString(), fee: 0, tax: 0,
      reason: "使用者確認已全部賣出",
    });
  };
    ...
    <section className="ai-section ranking-panel">
      ...
      {!candidates.length ? <div className="ai-monitor-empty">目前沒有達到 55 分的候選股票。</div>
        : <div className="table-scroll"><table className="ai-ranking-table"><thead><tr>
          ...
          <td><div className={`score-pill score-${Math.floor(row.score / 10)}`}>{row.score}</div></td>
...
function MonitorCard({ item, statusClass, onAnalyze, onConfirm, onIgnore, onRemove }: {
  item: AIStockMonitor; statusClass: (status: string) => string; onAnalyze: (symbol: string) => void;
  onConfirm: () => void; onIgnore: () => void; onRemove: () => void;
}) {
  return <article className="ai-monitor-card">
    ...
    <footer><button onClick={() => onAnalyze(item.symbol)}>查看分析</button><button className="primary" disabled={item.monitorStatus !== "buy_confirmed"} onClick={onConfirm}>我已買進</button><button onClick={onIgnore}>忽略訊號</button><button onClick={onRemove}>移除監控</button></footer>
  </article>;
}

function PositionCard({ item, statusClass, onAnalyze, onAddOn, onPartial, onClose, onContinue }: {
