...
STRATEGY_ROBOTS: tuple[dict[str, Any], ...] = (
    {
        ...
        "entryRule": "站穩 VWAP 並出現 5 分 K 突破，搭配量能與大單買盤確認。",
    ...
    },
    {
        ...
        "description": "偏多但不追價，以回撤 VWAP 或短均線的承接為主要買點。",
        "entryRule": "回撤 VWAP/均線不破並重新轉強，搭配 5 分 K 與大單確認。",
        "avoidRule": "跌破 VWAP 或回測失敗時停止做多。",
    },
    {
        "id": "range-two-way",
        "name": "震盪雙向機器人",
        ...
        "description": "盤勢沒有明確方向時，同時保留多空，但只在 VWAP、5 分 K 與大單方向一致後進場。",
        "entryRule": "站回 VWAP 做多；跌破 VWAP 並由 5 分 K 轉弱做空，必須通過大單確認。",
    ...
    },
    {
        ...
        "description": "偏空盤不追殺，等待反彈到 VWAP 或短均線附近轉弱後放空。",
        "entryRule": "反彈至 VWAP/短均附近轉弱，搭配 5 分 K 均線向下與賣壓確認。",
        "avoidRule": "急跌低檔不追空，站回 VWAP 即取消。",
    },
    {
        ...
        "description": "順著強空趨勢，以跌破 VWAP 或盤中低點後的回測失敗為主要空點。",
        "entryRule": "跌破 VWAP/盤中低點後等待 5 分 K 與賣壓確認放空。",
...
def strategy_context(
    ...
    if data_status not in {"normal", "closed"}:
        ...
        status, status_label = "paused", "12:00 後停止新進場・持倉管理中"
    elif phase == "long_only" and formal_allowed:
        status, status_label = "managing", "12:00 後停止新進場・持倉管理中"
