...
- 當沖機器人2回測中心可選取已匯入的 CSV／Parquet 分鐘資料集執行回測
- 獨立「強勢股策略」：盤後排行榜、市場與產業強度、模擬資金帳本、持倉、交易、績效與通知

## 強勢股策略資料邊界

強勢股策略使用既有 TWSE／TPEx／FinMind 日行情與現有基本面欄位，每個交易日盤後完成資料檢查後才更新排名。模擬資金固定使用獨立的 3,000,000 元帳本，不會和兩套當沖功能共用庫存，也沒有真實下單端點。

目前資料來源尚未提供可驗證的歷史上市／下市股票全集、產業分類沿革、除權息與減資事件、財報及月營收的實際公布時間、歷史成交容量，因此長週期回測會建立一筆 `DATA_INSUFFICIENT` 任務並列出缺少資料，不會產生虛構績效。當日資料完整度不足的股票會留在排行榜並顯示缺少欄位，但不會建立模擬委託。

當沖機器人2的回測不會使用即時掃描畫面代替歷史資料。先在策略優化中心匯入含時區、市場狀態與產業欄位的分鐘 CSV／Parquet，然後在回測中心選取該資料集。部署環境必須將 `DTV2_OPTIMIZATION_DATA_DIR` 指向持久化磁碟，否則上傳會被拒絕，避免資料在重啟後遺失。
...
主選單的「型態選股機器人」及獨立路由 `/pattern-robot` 使用獨立的 `pattern_*` 資料表、模擬帳戶、持倉、交易週期與權益曲線，不會併入 AI 當沖機器人績效。掃描母體限定為策展的 AI 核心與延伸供應鏈（伺服器、散熱、PCB／ABF、記憶體、CPO、先進封裝、電源及資料中心基建等），再讀取至少 180 個交易日真實行情並套用流動性與交易資格過濾。型態偵測使用還原權息價格；即時監控及模擬成交使用未還原市價。

後端預設會從 `ADAPTIVE_ELECTRONIC_SCANNER_URL` 推導 `/api/pattern-robot/scanner`，也可直接設定 `PATTERN_ROBOT_SCANNER_URL`。AI 相關選股池由後端 `/api/v1/pattern-robot/universe` 提供；前端無法取得該端點時只會縮小到本機策展清單，不會擴大成全市場。結果會快取並以 PostgreSQL advisory lock、每日 run 唯一限制及 signal/order/message 唯一限制防止多實例重複執行。
